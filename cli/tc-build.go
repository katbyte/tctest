...
func (f FlagData) BuildCmd(branch, testRegex, service string) error {
	tc := f.NewServer()

	c.Printf("triggering <magenta>%s</>%s @ <darkGray>%s...</>\n", branch, service, f.TC.Build.TypeID)

	buildID, buildURL, err := tc.RunBuild(f.TC.Build.TypeID, f.TC.Build.Parameters, branch, testRegex, f.TC.Build.SkipQueue)
	if err != nil {
...
func (f FlagData) TestResultsCmd(buildID string) error {
	tc := f.NewServer()
...
func (f FlagData) TestResultsByPRCmd(pr int) error {
	...
	builds, err := tc.BuildLocator(f.TC.Build.TypeID, pr, f.TC.Build.Latest, f.TC.Build.Wait, f.TC.Build.RunTimeout, f.TC.Build.RunTimeout)
	if err != nil {
