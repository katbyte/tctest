...
func (r DnsAAAARecordResource) uncompressed(data acceptance.TestData) string {
	return fmt.Sprintf(`
%s

resource "azurerm_dns_aaaa_record" "test2" {
  name                = "myaaaarecord2%d"
  resource_group_name = azurerm_resource_group.test.name
  zone_name           = azurerm_dns_zone.test.name
  ttl                 = 300
  records             = ["2607:f8b0:4009:1803:0000:0000:0000:1005"]
}
